...
namespace  Cinkes
{
	...
	class CTestEPA
	{
        ...
        static CVector3  pBuf[100];
        static CVector3  qBuf[100];
        static CVector3 yBuf[100];


        static Triangle* triangleHeap[200];
        ...
        inline void addCandidate(Triangle* triangle, CScalar upper2)
        {
        ...
        inline int originInTetrahedron(const CVector3& p1, const CVector3& p2,
            const CVector3& p3, const CVector3& p4)
        {
            CVector3 normal1 = CVector3::Cross((p2 - p1), (p3 - p1));
            if ((CVector3::Dot(normal1, p1) > CScalar(0.0)) == (CVector3::Dot(normal1, p4) > CScalar(0.0)))
            {
                return 4;
            }

            CVector3 normal2 = CVector3::Cross(p4 - p2, p3 - p2);
            if ((CVector3::Dot(normal2, p2) > CScalar(0.0)) == (CVector3::Dot(normal2, p1) > CScalar(0.0)))
            {
                return 1;
            }

            CVector3 normal3 = CVector3::Cross(p4 - p3, p1 - p3);
            if ((CVector3::Dot(normal3, p3) > CScalar(0.0)) == (CVector3::Dot(normal3, p2) > CScalar(0.0)))
            {
                return 2;
            }

            CVector3 normal4 = CVector3::Cross(p2 - p4, p1 - p4);
            if ((CVector3::Dot(normal4, p4) > CScalar(0.0)) == (CVector3::Dot(normal4, p3) > CScalar(0.0)))
            {
                return 3;
            }
        ...
        bool penDepth(const CTestGJK& gjk, const CCollisionObject& a, const CCollisionObject& b,
            CVector3& v, CVector3& pa, CVector3& pb)
        {

            int num_verts = gjk.getSimplex(pBuf, qBuf, yBuf);
            CScalar tolerance = DT_Accuracy::tol_error * gjk.maxVertex();
            ...
            case 2:
            {
                ...
                CVector3 aux1 = CVector3::Cross(dir, CVector3(axis == 0, axis == 1, axis == 2));
                CVector3 aux2 = rot_mat * aux1;
                CVector3 aux3 = rot_mat * aux2;

                pBuf[2] = a.GetCollisionShape()->Support(aux1);
                qBuf[2] = b.support(aux1 * -1);
                yBuf[2] = pBuf[2] - qBuf[2];

                pBuf[3] = a.support(aux2);
                qBuf[3] = b.support(aux2 * -1);
                yBuf[3] = pBuf[3] - qBuf[3];

                pBuf[4] = a.support(aux3);
                qBuf[4] = b.support(aux3 * -1);
            ...
            case 4:
            {
                ...
                if (bad_vertex == 0)
                {
                    ...
                    if (!(f0 && f0->getDist2() > CScalar(0.0) &&
                        f1 && f1->getDist2() > CScalar(0.0) &&
                        f2 && f2->getDist2() > CScalar(0.0) &&
                        f3 && f3->getDist2() > CScalar(0.0)))
                    {
                        return false;
                    }
            ...
            case 3:
            {
                ...
                CVector3 v1 = yBuf[1] - yBuf[0];
                CVector3 v2 = yBuf[2] - yBuf[0];
                CVector3 vv = CVector3::Cross(v1, v2);

                pBuf[3] = a.support(vv);
                qBuf[3] = b.support(vv * -1);
                yBuf[3] = pBuf[3] - qBuf[3];
                pBuf[4] = a.support(vv * -1);
                qBuf[4] = b.support(vv);
                yBuf[4] = pBuf[4] - qBuf[4];
                ...
                if (!(f0 && f0->getDist2() > CScalar(0.0) &&
                    ...
                    f2 && f2->getDist2() > CScalar(0.0) &&
                    f3 && f3->getDist2() > CScalar(0.0) &&
                    f4 && f4->getDist2() > CScalar(0.0) &&
                    f5 && f5->getDist2() > CScalar(0.0)))
                {
                    return false;
                }
            ...
            do
            {
                ...
                if (!triangle->isObsolete())
                {
                    ...
                    qBuf[num_verts] = b.support(triangle->getClosest() * -1);
                    yBuf[num_verts] = pBuf[num_verts] - qBuf[num_verts];

                    int index = num_verts++;
                    CScalar far_dist = CVector3::Dot(yBuf[index], triangle->getClosest());

                    // Make sure the support mapping is OK.
                    assert(far_dist > CScalar(0.0));
