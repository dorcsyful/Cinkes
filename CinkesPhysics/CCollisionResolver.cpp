...
Cinkes::CMat3x3 Cinkes::CCollisionResolver::CalculateContactBasis(const CInternalContactInfo* a_Info) const
{
	CVector3 tangent[2];
	if(CUtils::Abs(a_Info->m_Normal.getX()) > CUtils::Abs(a_Info->m_Normal.getY()))
	{
		CScalar scaling = static_cast<CScalar>(1.0) / 
					CUtils::Sqrt(a_Info->m_Normal.getZ() * a_Info->m_Normal.getZ() + a_Info->m_Normal.getX() * a_Info->m_Normal.getX());

		tangent[0][0] = a_Info->m_Normal.getZ() * scaling;
		tangent[0][1] = 0;
		tangent[0][2] = -a_Info->m_Normal.getX() * scaling;
	...
	{
		CScalar scaling = static_cast<CScalar>(1) /
			CUtils::Sqrt(a_Info->m_Normal.getZ() * a_Info->m_Normal.getZ() + a_Info->m_Normal.getY() * a_Info->m_Normal.getY());

		tangent[0][0] = 0;
		tangent[0][1] = a_Info->m_Normal.getZ() * scaling * -1;
		tangent[0][2] = a_Info->m_Normal.getY() * scaling;
...
void Cinkes::CCollisionResolver::StepOne(const std::vector<std::shared_ptr<CInternalContactInfo>>& a_Info, CScalar a_T) const
{
	for (const auto& current : a_Info)
	{
		current->m_ContactToWorld = CalculateContactBasis(current.get());
		current->m_RelativeContactPosition[0][0] = current->m_ContactPoints[0] - current->m_First->GetTransform().getOrigin();
		current->m_RelativeContactPosition[1][0] = current->m_ContactPoints[0] - current->m_Second->GetTransform().getOrigin();
		if (current->m_First->GetType() == EOBJECT_TYPE::TYPE_RIGID) {
			current->m_ContactVelocity = CalculateLocalVelocity(0, a_T, current.get());
		}
		if (current->m_Second->GetType() == EOBJECT_TYPE::TYPE_RIGID)
		{
...
void Cinkes::CCollisionResolver::StepTwo(const std::vector<std::shared_ptr<CInternalContactInfo>>& a_Info, CScalar a_T) const
{
	...
	{
		CScalar max_penetration = static_cast<CScalar>(0.0001);
		index = a_Info.size();
		...
		for(i = 0; i < a_Info.size(); i++)
		{
			for(unsigned b = 0; b < 2; b++)
			{
				...
				{
					CInternalContactInfo A = *a_Info[i];
					CInternalContactInfo B = *a_Info[index];
					if(A[b] == B[d])
					{
						CVector3 delta_position = linear_change[d] + angular_change[d].Cross(a_Info[i]->m_RelativeContactPosition[b][0]);
						a_Info[i]->m_PenetrationDepth = delta_position.Dot(a_Info[i]->m_Normal) * (b ? 1 : -1);
					}
				}
			}
		}
...
void Cinkes::CCollisionResolver::StepThree(std::vector<std::shared_ptr<CInternalContactInfo>>& a_Info, CScalar a_T) const
{
					...
					if (object_from_A == object_from_B)
					{
						deltavelocity = velocitychange[d] +
							rotationchange[d].Cross(
								a_Info[i]->m_RelativeContactPosition[b][0]);
...
{
	return  a_Object->GetTransform().Inverse() * a_Position;
}

Cinkes::CVector3 Cinkes::CCollisionResolver::ConvertToWorld(CCollisionObject* a_Object, const CVector3& a_Position) const
{
	return a_Object->GetTransform() * a_Position;
}


void Cinkes::CCollisionResolver::CalculateDeltaVelocity(CScalar a_T, CInternalContactInfo* a_Info) const
{
	CScalar velocityformacceleration = 0;
	CInternalContactInfo temp = *a_Info;
	CCollisionObject* object = temp[0];
	if (temp[0]->GetType() == EOBJECT_TYPE::TYPE_RIGID) {
	...
	if(temp[1]->GetType() == EOBJECT_TYPE::TYPE_RIGID)
	{
...
Cinkes::CVector3 Cinkes::CCollisionResolver::CalculateLocalVelocity(unsigned a_Index, CScalar a_T, CInternalContactInfo* a_Info) const
{
	CCollisionObject* temp[2] = { a_Info->m_First.get(),a_Info->m_Second.get() };
	CRigidBody* body = static_cast<CRigidBody*>(temp[a_Index]);
	CVector3 velocity = body->GetAngularVelocity().Cross(a_Info->m_RelativeContactPosition[a_Index][0]);
...
void Cinkes::CCollisionResolver::ApplyPositionChange(CVector3 a_LinearChange[2], CVector3 a_AngularChange[2], CScalar a_Penetration, CInternalContactInfo* a_Info) const
{
	CRigidBody* temp[2] = { nullptr,nullptr };
	if (a_Info->m_First->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[0] = static_cast<CRigidBody*>(a_Info->m_First.get()); }
	if (a_Info->m_Second->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[1] = static_cast<CRigidBody*>(a_Info->m_Second.get()); }
	...
	for(unsigned i = 0; i < 2; i++)
	{
		if(temp[i])
		{
			CScalar inverse_intertia = static_cast<CScalar>(1.0) / totalintertia;
			CScalar sign = (i == 0) ? 1 : -1;
			angularmove[i] = (sign * a_Penetration) * angularintertia[i] * inverse_intertia;
			linearmove[i] = (sign * a_Penetration) * linearintertia[i] * inverse_intertia;

			CVector3 projection = a_Info->m_RelativeContactPosition[i][0];
			CScalar toadd = (a_Info->m_RelativeContactPosition[i][0] * -1).Dot(a_Info->m_Normal);
			...
			{
				CVector3 tartgetangulardirection = a_Info->m_RelativeContactPosition[i][0].Cross(a_Info->m_Normal);
				CMat3x3 tensor = temp[i]->GetInverseInertiaTensorWorld();
...
void Cinkes::CCollisionResolver::ApplyVelocityChange(CInternalContactInfo* a_Info, CVector3 a_VelocityChange[2], CVector3 a_RotationChange[2]) const
{
	CRigidBody* temp[2] = { nullptr,nullptr };
	if (a_Info->m_First->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[0] = static_cast<CRigidBody*>(a_Info->m_First.get()); }
	if (a_Info->m_Second->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[1] = static_cast<CRigidBody*>(a_Info->m_Second.get()); }
	CMat3x3 inverseInertiaTensor[2];
	inverseInertiaTensor[0] = temp[0]->GetInverseInertiaTensorWorld();
	if (temp[1]) { inverseInertiaTensor[1] = temp[1]->GetInverseInertiaTensorWorld(); }
	...
	CVector3 impulsivetorque = a_Info->m_RelativeContactPosition[0][0].Cross(impulse);
	...
	if(temp[1])
	{
		impulsivetorque = impulse.Cross(a_Info->m_RelativeContactPosition[1][0]);
...
Cinkes::CVector3 Cinkes::CCollisionResolver::FrictionlessImpulse(CMat3x3 a_Tensor[2], CScalar a_DesiredVelocity, CInternalContactInfo* a_Info) const
{
	CRigidBody* temp[2] = { nullptr, nullptr };
	if (a_Info->m_First->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[0] = static_cast<CRigidBody*>(a_Info->m_First.get()); }
	if (a_Info->m_Second->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[1] = static_cast<CRigidBody*>(a_Info->m_Second.get()); }
	CVector3 impulsecontact;
	CVector3 deltavelocityworld = a_Info->m_RelativeContactPosition[0][0].Cross(a_Info->m_Normal);
	deltavelocityworld = a_Tensor[0] * deltavelocityworld;
	deltavelocityworld = deltavelocityworld.Cross(a_Info->m_RelativeContactPosition[0][0]);
	...
	if(temp[1])
	{
		deltavelocityworld = a_Info->m_RelativeContactPosition[1][0].Cross(a_Info->m_Normal);
		deltavelocityworld = a_Tensor[1] * deltavelocityworld;
		deltavelocityworld = deltavelocityworld.Cross(a_Info->m_RelativeContactPosition[1][0]);
...
Cinkes::CVector3 Cinkes::CCollisionResolver::FrictionImpulse(CMat3x3 a_Tensor[2], CScalar a_DesiredVelocity, CScalar a_Friction, CInternalContactInfo* a_Info) const
{
	CRigidBody* temp[2] = { nullptr, nullptr };
	if (a_Info->m_First->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[0] = static_cast<CRigidBody*>(a_Info->m_First.get()); }
	if (a_Info->m_Second->GetType() == EOBJECT_TYPE::TYPE_RIGID) { temp[1] = static_cast<CRigidBody*>(a_Info->m_Second.get()); }
	...
	impulseToTorque.setSkewSymmetric(a_Info->m_RelativeContactPosition[0][0]);
	...
	if (temp[1])
	{
		...
		impulseToTorque.setSkewSymmetric(a_Info->m_RelativeContactPosition[1][0]);
