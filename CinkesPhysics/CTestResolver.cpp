...
bool Cinkes::CTestResolver::Resolve(const std::vector<std::shared_ptr<CContactInfo>>& a_Info, CScalar a_T) const
{
	for (const auto& element : a_Info)
	{
		...
		CVector3 a_relative_position = element->m_ContactPoints[0] - element->m_First->GetTransform().getOrigin();
		CVector3 b_relative_position = element->m_ContactPoints[0] - element->m_Second->GetTransform().getOrigin();
