...
namespace Cinkes
{
	/**
	 * @ingroup CMat3x3
	 * @brief A 3x3 matrix, generally used to represent rotation
	 * @{
	 */
	class CMat3x3
	{
	public:
		...
		/**
		 * @brief Makes columns into rows and vice versa
		 */
		CMat3x3 Transpose();
		CMat3x3 GetInverse();
		/**
		 * @brief Static function to get the identity matrix. Do not call on member function!
		 */
		static CMat3x3 GetIdentity();
		CScalar Determinant();
		/**
		 *  @brief Gets the determinant of a 2x2 matrix. Used for calculating the inverse
		 */
